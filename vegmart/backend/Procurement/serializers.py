...
class CustomProcurementRawmaterialSerializer(serializers.ModelSerializer):
    ...
    procurement_item_unit_name = serializers.SerializerMethodField()
    def get_procurement_item_unit_name(self, obj):
        obj_id = obj.raw_material_id
        if obj_id is not None and obj_id !='' and obj_id !='None':
            try:
                obj = RawProductMaster.objects.filter(id=obj_id).first()
...
class RecipeRawmaterialSerializer(serializers.ModelSerializer):
    
    class Meta:
        model= RecipeRawmaterial
        fields='__all__'
class CustomRecipeRawmaterialSerializer(serializers.ModelSerializer):

    unit_name = serializers.SerializerMethodField()
    def get_unit_name(self, obj):
        obj_id = obj.raw_product_id
        if obj_id is not None and obj_id !='' and obj_id !='None':
            try:
                obj = RawProductMaster.objects.filter(id=obj_id).first()
                if obj is not None:
    ...
    raw_product_name = serializers.SerializerMethodField()
    def get_raw_product_name(self, obj):
        obj_id = obj.raw_product_id
        if obj_id is not None and obj_id !='' and obj_id !='None':
            try:
                obj = RawProductMaster.objects.filter(id=obj_id).first()
